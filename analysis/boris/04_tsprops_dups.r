...
dfproc <- function(path) {
  ...
  wdf %>%
    ...
    write.csv(paste0("baked/dups/tss/", id, "_ts.csv"), row.names = FALSE)
...
for (i in seq_len(len(list.files("baked/dups/bindos")))) {
  tryCatch({
    proc <- dfproc(paste0(
      "baked/dups/bindos/", list.files("baked/dups/bindos")[i]
    ))
    lista1[[i]] <- proc[[1]]
    lista2[[i]] <- proc[[2]]
  })
...
bind_rows(lista1) %>% write.csv("baked/dups/props.csv",  row.names = FALSE)
bind_rows(lista2) %>% write.csv("baked/dups/mega_ts.csv",  row.names = FALSE)
