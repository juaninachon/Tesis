...
bart <- cortest.bartlett(cor(x), n = nrow(x))$p.value < 0.05
kmo <- psych::KMO(x)$MSA
x_scaled <- scale(x)
...
tb_10 <- as.data.frame(loadings)
tb_10$comunalidades <- as.data.frame(communalities)[,1]
colnames(tb_10)[1:2] <- c("PC1", "PC2")
...
\phantomsection
\addcontentsline{lof}{figure}{{16.Gráfico de scree del análisis de componentes principales - muestra jardines}}
\label{fig:16}
...
&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;Los valores de las dos dimensiones obtenidas del PCA no se asociaron significativamente con ninguna de las variables de interés, de modo que no se tuvieron en cuenta en los análisis posteriores. 
...
loadings <- pca$rotation[, sfac] %*% diag(pca$sdev[sfac])
communalities <- rowSums(loadings^2)
tb_x0 <- as.data.frame(loadings)
tb_x0$comunalidades <- as.data.frame(communalities)[,1]
colnames(tb_x0)[1] <- c("PC1")
...
En este caso, el PCA de los puntajes de la sesión 1 de la batería de FFEE derivó en una única dimensión (KMO = `r kmo`; Bartlett ~*p-val*~ > 0.001) que explica el `r pvar`% de la varianza en esta muestra (ver \hyperref[tab:x0]{Tabla x0} y Figuras \hyperref[fig:18]{18} y \hyperref[fig:19]{19}).

\phantomsection
\addcontentsline{lot}{table}{{x0. Análisis de componentes principales (cargas y comunalidades) -  muestra museo}}
\label{tab:x0}
**Tabla x0**  
...
\phantomsection
\addcontentsline{lof}{figure}{{18.Gráfico de scree del análisis de componentes principales - muestra museo}}
\label{fig:18}
