...
&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;Por otro lado, los estudios con participantes de edad preescolar han medido el tiempo que el estudiantado pasa en distintos tipos de actividades (e.g., periodos libres de instrucción, transiciones, juego con pares) y su nivel de involucramiento durante actividades de enseñanza-aprendizaje. También han puntuado globalmente indicadores vinculados al funcionamiento ejecutivo, la regulación emocional y la calidad del clima áulico, contrastando las medidas observacionales con pruebas estandarizadas de desempeño académico, gratificación aplazada, FFEE y el reporte docente de habilidades de aprendizaje, habilidades sociales y problemas de conducta. Estos estudios han encontrando variabilidad intraindividual en la conducta de los niños/as según el tipo de actividad observada. También han reportado asociaciones significativas entre las trayectorias de desempeño académico, el funcionamiento ejecutivo, el reporte docente, el clima áulico y la cantidad de tiempo que las niñas/os pasan desinvolucrados [@brockDelayGratificationFirst2014; @mccoyObservationalMeasureRegulationRelated2017; @moffettOfftaskBehaviorKindergarten2020; @rimm-kaufmanContributionChildrensSelfregulation2009; @slotPreschoolersCognitiveEmotional2017]. 
...
(A) Desarrollar y poner a prueba un sistema de observación ecológica del comportamiento autorregulatorio en contextos cotidianos de juego y aprendizaje.
...
(C) Analizar asociaciones entre el funcionamiento autorregulatorio (valorado a través de pruebas de laboratorio, reportes y la codificación de observaciones) y el desempeño académico. 

(D) Identificar y analizar moduladores individuales (e.g., edad, sexo) y contextuales (i.e., condiciones de vida, recursos y estresores sociales del hogar, y características de los procesos áulicos), sobre las asociaciones entre medidas del funcionamiento autorregulatorio y del desempeño académico.
...
scols <- c(4, 7, 10, 13) # c(4:5, 7:8, 10:11, 13, 15) #
srows <- mergej |>
  select(all_of(scols)) |>
  complete.cases()
x <- mergej |>
...
En primer lugar, se realizó un análisis de componentes principales con los puntajes de las pruebas *Toca botón*, *No encaja*, *Juego de memoria* y *Bloques de corsi (orden directo)*, del cual se obtuvieron dos componentes que de manera combinada explican el `r pvar`% de la varianza en los puntajes de estas pruebas (37% y 26%, respectivamente) (ver Figuras ta y da).

\phantomsection
\addcontentsline{lof}{figure}{{x.Gráfico de scree del análisis de componentes principales - muestra jardines}}
\label{fig:x}

**Figura x**  
*Gráfico de scree del análisis de componentes principales - muestra jardines.*
```{r}
#| label: scree j
#| fig-align: 'center'
#| fig.height: 5
...
\phantomsection
\addcontentsline{lof}{figure}{{y. Gráfico de dispersión (biplot) de los componentes principales - muestra jardines}}
\label{fig:y}
...
colnames(mergem)[163] <- "NeuroUX - PCA"
...
En este caso, el análisis de componentes principales de los puntajes de la sesión 1 de la batería de FFEE derivó en una única dimensión que explica el `r pvar`% de la varianza en esta muestra (ver Figuras pa y ca).

\phantomsection
\addcontentsline{lof}{figure}{{x.Gráfico de scree del análisis de componentes principales - muestra museo}}
\label{fig:x}

**Figura z**  
*Gráfico de scree del análisis de componentes principales - muestra museo.*
```{r}
#| label: scree m
#| fig-align: 'center'
#| fig.height: 5
...
\phantomsection
\addcontentsline{lof}{figure}{{y. Gráfico de dispersión (biplot) de los componentes principales - muestra museo}}
\label{fig:y}
...
lower_tri <- corr_formatted
lower_tri[upper.tri(lower_tri, diag = FALSE)] <- NA
lower_tri <- as.data.frame(lower_tri)
lower_tri <- tibble::rownames_to_column(lower_tri, var = ".")
cat("\\scriptsize\n")
Hmisc::latex(
...
m5m <- lm(data = clean_names(mergem), compuesto_woodcock ~ neuro_ux_pca + cbq_esfuerzo_de_control + obs_afec_neutra_nino_a)
lmtable(m5m)
```
...
&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;En el caso del modelo de tercer orden, los únicos predictores que conservaron su significancia fueron la edad de los niños/as (β = .44 , p < .001), el puntaje del PCA (β = -.303, p = .003) y el nivel educativo materno (β = .202 , p < .015) (ver \hyperref[tab:32]{Tabla 32}).
...
bam <- clean_names(mergem) |>
  select(
    compuesto_woodcock, neuro_ux_pca, cbq_esfuerzo_de_control, obs_afec_neutra_nino_a,  edad_anos, educacion_de_la_madre
  )
bam <- filter(bam, complete.cases(bam))

m6m <- lm(
  data = bam,
  compuesto_woodcock ~ neuro_ux_pca
)

m7m <- lm(
  data = bam,
  compuesto_woodcock ~ neuro_ux_pca + edad_anos + educacion_de_la_madre
)

m8m <- lm(
  data = bam,
  compuesto_woodcock ~ neuro_ux_pca + edad_anos + educacion_de_la_madre + cbq_esfuerzo_de_control + obs_afec_neutra_nino_a
)
...
&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;La comparación de los modelos anidados señala que la inclusión de ambos términos mejora el ajuste de cada nivel que lo antecede, pero el cambio con la introducción del nivel de la conducta en contexto ecológico no alcanza a ser significativo (ver \hyperref[tab:33]{Tabla 33}).
...
&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;Finalmente, en los modelos de interacciones simples entre el predictor más fuerte de cada tipo de instrumento, la edad de los niños/as y el nivel de educación materna (ver tablas 34 a 39), no se verificaron efectos de moderación, únicamente atenuación del reporte de esfuerzo de control por inclusión de la edad de los niños/as como covariable (ver tabla). 
...
m5m <- lm(
  formula = compuesto_woodcock ~ neuro_ux_pca + edad_anos + neuro_ux_pca * edad_anos,
  data = bmi
) 

m6m <- lm(
  formula = compuesto_woodcock ~ neuro_ux_pca + educacion_de_la_madre + neuro_ux_pca * educacion_de_la_madre,
  data = bmi
)
