...
&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;La tesis se organiza en doce apartados: el *resumen* &mdash;que anticipa el tema, los objetivos, métodos y resultados&mdash;, una breve sección de *agradecimientos*, la *introducción* &mdash;que racionaliza la propuesta y describe su recorrido&mdash;, el *marco teórico* &mdash;donde se exponen los conceptos centrales y se detallan los antecedentes empíricos relevantes que hacen al estado del arte&mdash;, el apartado donde se explicitan los *objetivos e hipótesis*, el apartado de *metodología* &mdash;donde se detallan los procedimientos, instrumentos y análisis que hacen a cada uno de los estudios&mdash;, los *resultados* obtenidos; la *discusión* &mdash;en la que se revén los resultados a la luz de los objetivos, hipótesis y antecedentes&mdash;, un apartado de *limitaciones y direcciones futuras,* el apartado de *conclusiones* &mdash;donde se sintetizan y valoran los hallazgos&mdash; y las referencias bibliográficas. En último lugar aparecen una sección de *materiales suplementarios* con enlaces a instrumentos de evaluación, protocolos, cuestionarios, herramientas para la exploración de los datos, el código de los análisis estadísticos y documentación que respalda aspectos éticos y administrativos
...
Analizar las asociaciones entre distintos abordajes de evaluación del funcionamiento autorregulatorio (i.e., tareas de laboratorio, reportes de adultos y observaciones ecológicas) y el desempeño académico durante la transición a la escolaridad primaria, y explorar su modulación por factores individuales y contextuales.
...
(3) Se espera que las aspectos individuales (e.g., edad) y contextuales (e.g., nivel socioeconómico) modulen la magnitud de la asociación entre funcionamiento autorregulatorio y desempeño académico. 
...
pk <- read.csv("data/raw/submuestras.csv") |>
 filter(inc == 1)
...
```{r}
#| label: ema
ema <- read.csv("data/raw/socdemo.csv")
...
docentes <- read.csv(
  "data/raw/docentes_demografía.csv",
  check.names = FALSE
) |> select(-c(1:3))
...
obsj <- left_join(
  mj,
  read.csv("data/raw/obsj/obs_keys_jardines.csv"),
  by = "ID"
)
...
dups <- left_join(
  mj,
  bind_rows(
    (read.csv("data/raw/obsj/nrioa24.csv") |>
      mutate(ID = str_split_i(IDNA, "_", i=3))),
    (read.csv("data/raw/obsj/nrioa23.csv") |>
...
nrat23 <- read.csv("data/raw/obsj/nrat23.csv") |> slice(1:394)
nrat24 <- read.csv("data/raw/obsj/nrat24.csv") |> slice(1:653)
...
repj <- read.csv("data/raw/reportesywm.csv") |>
  filter(ID  %in% mj$ID)
wmj <- repj |> select(1:3)
...
nuxj <- read.csv("data/raw/nux.csv") |>
  filter(ID %in% mj$ID) |>
  select(-c(
...
nioa23 <- read.csv("data/raw/obsj/nioa23.csv")
nrioa23 <- read.csv("data/raw/obsj/nrioa23.csv")
nioa24 <- read.csv("data/raw/obsj/nioa24.csv")
nrioa24 <- read.csv("data/raw/obsj/nrioa24.csv")
...
cbq3 <- read.csv("data/raw/cbqe23.csv")
cbq4 <- read.csv("data/raw/cbqe24.csv")
...
chaosj <- read.csv("data/raw/chaosh.csv") |>
  filter(ID %in% mj$ID) |>
  arrange(ID) |>
  filter(!duplicated(ID)) |>
  select(-CHAOSC)

lecp3 <- read.csv("data/raw/chaosd23.csv") |>
  select(-c(X, Check, ID_x, CHAOSD))
lecp4 <- read.csv("data/raw/chaosd24.csv") |>
...
keys <- rbind(
  read.csv("data/raw/keysd23.csv"),
  read.csv("data/raw/keysd24.csv")
)
ecp <- rbind(
  read.csv("data/raw/ecp_raw23.csv"),
  read.csv("data/raw/ecp_raw24.csv")
)
ecp <- left_join(
  keys,
  ecp,
...
```{r}
#| label: desc obs m 
obsm <- read.csv("data/raw/obsm/museo_props.csv")  |>
  filter(ID %in% mm$ID)
...
mdiar <- left_join(
  mm,
  read.csv("data/raw/obsm/elan.csv"),
  by = "ID"
) |> 
  left_join(
    select(obsm, 1:5)
...
mturnos <- read.csv("data/raw/obsm/turnos_conv.csv") |>
filter(ID %in% mm$ID)
...
mpyannote <- left_join(
  mm,
  read.csv("data/raw/obsm/diarización.csv"),
  by = "ID"
) |> select(-c(2,3)) |>
  left_join(
    select(obsm, 1:5)
...
mclan <- read.csv("data/raw/obsm/clan.csv") |>
  filter(ID %in% mm$ID)
mclanid <- mclan$ID
...
mfeatgaze <- reduce(
  c(list(
    select(mm,ID),
    read.csv("data/raw/obsm/propfeat.csv"),
    read.csv("data/raw/obsm/propgazelle.csv")
  )),
  left_join, by = "ID"
)
...
repm <- read.csv("data/raw/reportesywm.csv") |>
  filter(ID  %in% mm$ID)
wmm <- repm |> select(1:3)
...
nuxm <- read.csv("data/raw/nux.csv") |>
  filter(ID %in% mm$ID) |>
  select(-c(
    tb_errors, ne_errors, mt_errors, jm_tot_test_trials, 
    jm_errors, jm_complete, jm_tot_ax_trial, jm_ax_score, jm_ax_errors, jm_tot_else_trial, jm_else_score, jm_else_errors, jm_ax_meanrt_correct, jm_ax_meanrt_incorrect, jm_else_meanrt_correct, jm_else_meanrt_incorrect
...
for (i in seq_along(dups)){
  a[[i]] <- read.csv(paste0("data/raw/obsm/tsboris/", dups[i], "_ts.csv")) |>
    filter(fase  %in% fases) |>
    select(4:32)
  b[[i]] <- read.csv(paste0("data/raw/obsm/tsboris/", dups[i], "_ts.csv")) |>
    filter(fase  %in% fases) |>
    select(4:32)
}
...
a <- read.csv("data/raw/obsm/propsboris.csv")
b <- read.csv("data/raw/obsm/propsborisdups.csv") |> 
  filter(id  %in% a$id)
a <- a |>
  filter(id  %in%  b$id) |>
  select(-c(1:6))
...
guide <- read.csv("data/raw/obsm/syncsub.csv") |>
  left_join(read.csv("data/raw/obsm/sync.csv"))
elans <- guide$ID[which(guide$ELAN==1)]
paths_elan <- list.files("data/raw/obsm/tselan", full.names = TRUE)
paths_pyannote <- list.files("data/raw/obsm/pyannotedups", full.names = TRUE)
mmegats <- do.call(rbind, lapply(paste0("data/raw/obsm/museo_mega_ts", 1:4, ".csv"), read.csv))
tsfeat <- read.csv("data/raw/obsm/tsfeat2.csv")
paths_gazelle <- list.files("data/raw/obsm/gazellesync", full.names = TRUE)
...
for (i in 1:len(elans)) {
  tryCatch(
    {
      Id <- elans[i]
      boris <- read.csv(paste0("data/raw/obsm/tsboris/", Id, "_ts.csv")) |> select(fase)
...
cbqm <- read.csv("data/raw/cbqm.csv") |>
  filter(ID %in% mm$ID) |>
  arrange(ID) |>
  filter(!duplicated(ID)) 
  
chaosm <- read.csv("data/raw/chaoshm.csv") |>
  filter(ID %in% mm$ID) |>
  arrange(ID) |>
  filter(!duplicated(ID)) |>
  select(-CHAOSC)
